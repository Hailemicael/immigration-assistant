...
func dowloadContent(url, dir, name string, retryCount int) (err error) {
	if retryCount <= 0 {
		return
	}
	...
	if resp.StatusCode != http.StatusOK {
		...
		return dowloadContent(url, dir, name, retryCount-1)
	}
...
func crawlImmigrationForms(parentDir string) *colly.Collector {
	...
	c.OnHTML("span.file.file--mime-application-pdf.file--application-pdf", func(e *colly.HTMLElement) {
		...
		err = dowloadContent(link.String(), dir, name, 4)
		if err != nil {
...
func crawLegistlation(parentDir string) *colly.Collector {
	...
	c.OnHTML("tr", func(e *colly.HTMLElement) {
		...
		err = dowloadContent(metadata.Link, dir, fmt.Sprintf("%s-%s.xhtml", metadata.Act, metadata.Code), 4)
		if err != nil {
